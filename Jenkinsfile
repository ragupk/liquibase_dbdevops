...
agent { label 'master' }
    ...
    "DBNAME=${AllConfig['DB_NAME']}"]) {
      ...
      sh 'sh upgrade.sh'
       sh """
          if [ ! -z "$GIT_REPO" ]; 
          then git config --global user.email "jenkins@example.com" && git config --global user.name "Jenkins" && git clone --depth 1 --branch "$GIT_BRANCH" "$GIT_REPO" tag && { cd tag || exit 1 ; } && 
